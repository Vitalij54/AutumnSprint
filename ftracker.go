...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	rS := runningCaloriesMeanSpeedMultiplier
	rC := runningCaloriesMeanSpeedShift
	return (float64(rS) * speed * rC) * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	wM := walkingCaloriesWeightMultiplier
	wH := walkingSpeedHeightMultiplier
	speed := meanSpeed(action, duration)
	speedMtSec := speed * kmhInMsec
	return (wM*weight + (math.Pow(speedMtSec, 2)/(height/cmInM))*wH*weight) * duration * minInH
}
